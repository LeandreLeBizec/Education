...
and term_list((next, peek, junk) as tokens) =
  let t = term tokens in
    ...
    | COMMA ->
      junk();
      let l = term_list tokens in t::l
    | _ -> [t]

and predicate ((next, peek, junk) as tokens) = 
  match next() with
  | NAME name ->
    if next() <> LEFT_PAREN then error "missing ("
    else
      let tl = term_list tokens in
        if next() <> RIGHT_PAREN then error "missing )"
        else Predicate(name,tl)
  | _ -> error "no name"
...
and predicate_list ((next, peek, junk) as tokens) = 
  let p = predicate tokens in
    ...
    | COMMA ->
      junk();
      let l = predicate_list tokens in p::l
    | DOT -> 
      junk(); 
      [p]
    | _ -> error "missing ."
