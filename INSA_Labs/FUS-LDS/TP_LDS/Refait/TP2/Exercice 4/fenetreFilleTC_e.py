...
class FenetreFilleTC(QMdiSubWindow):
    ...
    def roberts(self):
        ...
        for y in range(self.image.height()-1):
            for x in range(self.image.width()-1):
                ...
                nbg=int(abs(pixel1-pixel4) + abs(pixel2-pixel3))
    ...
    def seuil(self,n):
        ...
        for i in range(0,FenetreFilleTC.LUM_MAX+1):
            if(n <= i):
                res = FenetreFilleTC.LUM_MAX
            else:
                res = 0
            self.image.setColor(i, qRgb(res, res, res))
        # Recopie de l'image avec la nouvelle table des couleurs dans la fenêtre
        self.recopieImage(self.image)
        return(self)
    ...
    def floyd(self):
        ...
        for y in range(self.image.height()-1):
            ...
            for x in range(1,self.image.width()-1):
                ...
                for (dy, dx, prop) in [[1,0,7], [-1,1,3], [0,1,5], [1,1,1]]:
                    if y+dy < 0 or x+dx < 0:
                        continue
                    nvv=max(0,min(fille.image.pixelIndex(x+dx,y+dy)+prop*err/16,255),0)
                    fille.image.setPixel(x+dx,y+dy,int(nvv))
    ...
    def bitPoidsFaible(self,n):
        for i in range(FenetreFilleTC.TAILLE_TDC):
            ...
            v=[0.255][i & 1]
            self.image.setColor(i, qRgb(v, v, v))
        # Recopie de l'image avec la nouvelle table des couleurs dans la fenêtre
        self.recopieImage(self.image)
        return(self)
