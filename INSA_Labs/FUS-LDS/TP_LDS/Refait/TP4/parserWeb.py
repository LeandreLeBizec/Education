...
class RechercheBR:
    ...
    def analyseURL(self,url):
        ...
        if (ean!=None):
            ...
            res = re.compile("^(.*) \((\d{4})\) - (.*)").search(result)
            self.titre=res.group(1)
            self.annee=res.group(2)
            self.type=res.group(3)
            # Récupération du champ duree quand il existe
            res=soups.find(string=re.compile("<time datatime=\"P\d+M\">(.*)</time>"))
            self.duree = res.group()
            self.trouve=True
