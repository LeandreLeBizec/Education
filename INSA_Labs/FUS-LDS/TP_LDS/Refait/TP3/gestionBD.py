...
class BD(BDRegMat):
    ...
    def personneExistence(self, ident):
        ...
        try:
            ...
            # Q8 ...
            table = self.question("SELECT * FROM personnes WHERE ident = '{}'".format(ident))
            if (len(table) > 0):
                # Retour du résultat de la requête
                # Q8 ...
                res = 1
                # self.out.afficher("{} est dans la base".format(ident))
    ...
    def personneCode(self, ident):
        ...
        try:
            ...
            table = self.question("SELECT sexe, nom, prenom, profession, dateNaissance, commune, personnes.codeINSEE "
                                  "FROM lieux,personnes WHERE personnes.codeINSEE = lieux.codeINSEE AND  ident = '{}'".format(ident))
            if len(table) > 0:
                if len(table) == 1:
    ...
    def personneInsertion(self, t):
        ...
        if t[6] != 0:
            ...
            self.requete(
                "INSERT INTO personnes(ident,sexe,nom,prenom,profession,dateNaissance) VALUES('{0}','{1}','{2}',"
                "'{3}','{4}','{5}')".format(t[0], t[1], t[2], t[3], t[4], t[5]))

    # Mise à jour d'une personne
    def personneMiseAJour(self, t):
        if t[5] != 0:
            self.requete(
                "UPDATE personnes SET sexe='{0}',nom='{1}',prenom='{2}',profession='{3}',dateNaissance='{4}',"
                "codeINSEE={5} WHERE ident='{6}'".format(t[0], t[1], t[2], t[3], t[4], t[5], t[6]))
        else:
            self.requete(
                "UPDATE personnes SET sexe='{0}',nom='{1}',prenom='{2}',profession='{3}',dateNaissance='{4}',"
                "codeINSEE=NULL WHERE ident='{5}'".format(t[0], t[1], t[2], t[3], t[4], t[6]))

    # Effacement d'une personne
    def effacerPersonne(self, ident):
        self.requete("BEGIN")
        # Q12 ...
        self.requete("DELETE FROM resider WHERE ident='{}'".format(ident))
        self.requete("DELETE FROM personnes WHERE ident='{}'".format(ident))
        self.requete("END")

    # Insertion d'une nouvelle personne
    def residenceInsertion(self, t):
        # Q14 ...
        self.requete("INSERT INTO redider VALUES('{}', '{}', '{}')".format(*t))

    # Effacement d'un lieu de résidence pour une personne
    def effacerResidence(self, t):
        # Q15 ...
        self.requete("DELETE FROM resider WHERE ident='{}' AND codeINSEE='{}' and date='{}'".format(*t))
    ...
    def lieuCode(self, lieu):
        ...
        try:
            # Recherche des militaires
            # Q7 ...
            table = self.question("SELECT codeINSEE FROM lieux WHERE nomCommune='{}'".format(lieu))
            if (len(table) > 0):
                if (len(table) == 1):
                    # Retour du code du lieu
                    res = str(table[0][0])
                    self.out.afficher("Commune : {} code ({}) \n".format(lieu, res))
                else:
    ...
    def listeMilitaires(self):
        ...
        try:
            # Recherche des militaires
            # Q11 ...
            table = self.question("SELECT ident, nom, prenom, dateNaissance, nomCommune FROM personnes, lieux "
                                  "WHERE lieux.codeINSEE = personnes.codeINSEE")
            if (len(table) > 0):
                self.out.afficher("Nombre d'individus : {0}\n".format(len(table)))
                # Affichage de chaque militaire
                for personne in table:
                    self.out.afficher("M. {0} {1}, né le {2} à {3} \n".format(personne[1], personne[2], personne[3], personne[4]))
            else:
                self.out.afficher(
