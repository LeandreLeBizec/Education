...
class Automate:
    ...
    def analyser(self, nom):
        f=open(nom, 'r')
        # la boucle de balayage de lecture des lignes du fichier
        for ligne in f:
            for i in range(len(self.automateDon[self.etat])):
                # Recherche des expressions régulières disponibles pour l'état courant
                regexp=re.compile(self.automateDon[self.etat][i][0])
                result=regexp.search(ligne)
                # le test de correspondance de la ligne avec l’expression régulière
                if result != None:
