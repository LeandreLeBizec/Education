...
make_all_pairs(L,Res):-
    make_all_pairs(L, L, L, Res).

make_all_pairs([],L,L,[]).
make_all_pairs([T|Q],[],L,Res):-
    make_all_pairs(Q,L,L,Res).
make_all_pairs([T1|Q1],[T2|Q2],L,[likes(T1,T2)|Res]):-    
    make_all_pairs([T1|Q1],Q2,L,Res).
...
proposition4(L):-
    not((member(likes(A,B),L), not(member(likes(B,A),L)))).
