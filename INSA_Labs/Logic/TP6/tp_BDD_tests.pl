...
%2.6
%!(je fournis tous les produits de lyon)=(je ne fourni pas au moins un produit de lyon)
fournisseurPasBon(NumF):- 
        fournisseurReference(NumF,NomF,Ville),
        piece(NumPiece, NomP, lyon),
        not(livraison(NumF,NumPiece,Quantite)).

division(NumF):-
        fournisseurReference(NumF,NomF,Ville),
        not(fournisseurPasBon(NumF)).

%2.7
findall(NumF, livraison, Liste)
