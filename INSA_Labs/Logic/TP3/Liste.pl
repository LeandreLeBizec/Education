...
test_compte_autres_modes :-  % Non demandé par l'énoncé
    assert_true( sortall(A, compte(A, [a,c,a,b,a,c,b], 2), [b,c]) ),    % compte(-A,+X,+N) : production des solutions A
    assert_true( sortall([A,N], compte(A,[b,c,a,b,a,a],N), [[a,3],[b,2],[c,1]]) ). % compte(-A,+X,-N) production A et N
...
test_conc3 :-
    ...
    assert_true( sortall([L1,L2,L3], conc3(L1,L2,L3,[1,2]), 
...
test_elim :-
    ...
    assert_true( not(elim([a,b,a,b,a], [a,b,b])) ),
...
compte(X, [T|Q], N):-
    compte(X,Q,N),
    X\=T.
% \= (non unifiable ) \==(difference)

%renverser(+X,?Y) Y est la liste renversé de X
renverser(X,Y):-
    renverser(X,[],Y).
%transforme une clause 2 arguments en 3 arguments pour ajouter la liste auxiliaire
renverser([],A,A).
renverser([X|Q],A,R):-
    renverser(Q,[X|A],R).
...
%enieme(+N, +L, -A) A est l'element de rang N de la liste L
enieme(1,[T|Q],T).
enieme(N,[T|Q],A):-
    N\==1,
    M is N-1,
...
%hors_de(+A,+L) A n'est pas un element de la liste L
hors_de(X, []).
hors_de(X, [T|Q]):-
    X \== T,
    hors_de(X, Q).
...
%tous_diff(+L) les elements de L sont tous differents
tous_diff([]).
tous_diff([T|Q]):-
    hors_de(T, Q),
    tous_diff(Q).
...
%conc3(+L1, +L2, +L3, ?L) L est la concatenation de L1 L2 et L3
conc3([],[],L,L).
conc3([],[X|L2],L3,[X|L]):-
    conc3([],L2,L3,L).    
...
%debute_par(+L1, ?L2) Liste L1 commence par la liste L2
debute_par(L1,[]).
debute_par([X|L1],[X|L2]):-
    debute_par(L1,L2).

%sous_liste(+L1,?L2) La liste L2 est une sous liste de la la liste L1
sous_liste(L1,[]).
...
%elim(+X,-Y) La liste Y est la liste X sans doublons
elim([],[]).
elim([X|L1],L2):-
    elim(L1,L2),
    membre(X,L2).
...
%inserer(+E,+L1,-L2) insere E a sa place dans la liste L1 deja trié pour produire la liste L2
inserer(E, [], [E]).            %liste vide
inserer(E, [E|L1], [E|L2]).     %si deja present
inserer(E, [X|L1], [X|L2]):-
    inserer(E, L1, L2).
inserer(E, [X|L1], [E|L2]):-
    inserer(E, L1, [E|L2]).
...
%tri(+L1,-L2) La liste Y est la liste X triée par ordre croissant.